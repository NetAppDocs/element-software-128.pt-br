...
summary: Você pode usar o método ListVolumeQoSHistograms para gerar um histograma do uso de QoS de volume para um ou vários volumes.  Isso permite que você entenda melhor como os volumes estão usando o QoS. 
---
...
[role="lead"]
Você pode usar o `ListVolumeQoSHistograms` Método para gerar um histograma do uso de QoS de volume para um ou vários volumes.  Isso permite que você entenda melhor como os volumes estão usando o QoS.
...
IDs de volume
 a| 
Uma lista opcional de IDs de volume especificando em quais volumes devem ser gerados histogramas de QoS.
 a| 
matriz de inteiros
 a| 
Nenhum
