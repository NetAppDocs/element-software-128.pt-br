---
permalink: api/reference_element_api_getnodesslcertificate.html 
sidebar: sidebar 
keywords: ssl,cert,certificate,get,getting,getnodesslcertificate,active 
summary: Você pode usar o método GetNodeSSLCertificate para recuperar o certificado SSL que está atualmente ativo no nó de gerenciamento. 
---
= Obter certificado SSL do nó
:allow-uri-read: 
:icons: font
:imagesdir: ../media/


[role="lead"]
Você pode usar o `GetNodeSSLCertificate` Método para recuperar o certificado SSL que está atualmente ativo no nó de gerenciamento.



== Parâmetros


NOTE: Você deve chamar esse método no nó de gerenciamento. Por exemplo:

[listing]
----
https://<management node IP>:442/json-rpc/10.0
----
Este método não possui parâmetros de entrada.



== Valores de retorno

Este método tem os seguintes valores de retorno:

|===
| Nome | Descrição | Tipo 


 a| 
certificado
 a| 
O texto completo do certificado codificado em PEM.
 a| 
corda



 a| 
detalhes
 a| 
As informações decodificadas do certificado.
 a| 
objeto JSON

|===


== Exemplo de solicitação

As solicitações para esse método são semelhantes ao seguinte exemplo:

[listing]
----
{
    "method" : "GetNodeSSLCertificate",
    "params" : {},
    "id" : 1
}
----


== Exemplo de resposta

Este método retorna uma resposta semelhante ao exemplo a seguir:

[listing]
----
{
    "id": 1,
    "result": {
        "certificate": "-----BEGIN CERTIFICATE-----\\n-----END CERTIFICATE-----\n",
        "details": {
            "issuer": "/C=US/ST=NV/L=Denver/O=NetApp/emailAddress=test@netapptest.org",
            "modulus": "",
            "notAfter": "2027-03-06T22:50:26Z",
            "notBefore": "2017-03-08T22:50:26Z",
            "serial": "CC1B221598E37FF3",
            "sha1Fingerprint": "1D:70:7A:6F:18:8A:CD:29:50:C7:95:B1:DD:5E:63:21:F4:FA:6E:21",
            "subject": "/C=US/ST=NV/L=Denver/O=NetApp/emailAddress=test@netapptest.org"
        }
    }
}
----